<script>
	...
	function realizarGestion() { //tipo deposito, efectivo
...
>
    <button
        ...
        class="px-6 py-2 font-bold rounded-md bg-purple-500 text-white hover:bg-purple-600 focus:outline-none disabled:bg-gray-400 disabled:cursor-not-allowed"
        disabled={
            gestion === 'deposito'
                ? !numeroCuenta || (monto === 0 || !monto) || !metodoDeposito
                : gestion === 'retiro'
                ? (monto === 0 || !monto) || !tipoRetiro
                : true
        }
    >
        Realizar Gestión
