<script>
    ...
    function generacionPDF(message) {
        try {
            ...
            doc.setFont("helvetica", "normal");
            doc.text($user.no_cuenta.toString(), 60, 40);

            doc.setFont("helvetica", "bold");
            doc.text("Tipo de transacción:", 10, 50);
            doc.setFont("helvetica", "normal");
            doc.text('Pago Prestamo', 60, 50);
    ...
    function GestionEmpezar() {
        fetch(`${PathBackend}/client/pago_prestamo`, {
            ...
            body: JSON.stringify({ cui_enc: 0, cuenta: $user.no_cuenta, id_prestamo: numeroPrestamo, monto: monto })
        })
        .then(res => {
            if(!res.ok) {
        ...
        <div class="rounded-t mb-0 px-0 border-0">
            <div class="flex flex-wrap items-center px-4 py-2">
                <div class="relative w-full max-w-full flex-grow flex-1">
                    <h3 class="font-semibold text-base text-gray-900 dark:text-gray-50">Pagar Préstamo</h3>
                </div>
            </div>
